  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  // NEW: optional IMU sensor name. If the URDF doesn't declare a <sensor>
  // block, leave this empty and no IMU interfaces get exported.
  if (info_.hardware_parameters.count("imu_sensor_name") > 0)
  {
    cfg_.imu_sensor_name = info_.hardware_parameters["imu_sensor_name"];
  }
  else if (info_.sensors.size() > 0)
  {
    // Fall back to whatever <sensor name="..."> was declared in the URDF.
    cfg_.imu_sensor_name = info_.sensors[0].name;
  }
...
std::vector<hardware_interface::StateInterface> DiffDriveArduinoHardware::export_state_interfaces()
{
  ...
  // NEW: IMU state interfaces, matching the naming the IMUSensor semantic
  // component (and therefore imu_sensor_broadcaster) expects:
  // <sensor_name>/orientation.{x,y,z,w}, .angular_velocity.{x,y,z},
  // .linear_acceleration.{x,y,z}
  if (!cfg_.imu_sensor_name.empty())
  {
    const auto & n = cfg_.imu_sensor_name;
    state_interfaces.emplace_back(hardware_interface::StateInterface(
      n, "orientation.x", &imu_orientation_[0]));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
      n, "orientation.y", &imu_orientation_[1]));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
      n, "orientation.z", &imu_orientation_[2]));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
      n, "orientation.w", &imu_orientation_[3]));

    state_interfaces.emplace_back(hardware_interface::StateInterface(
      n, "angular_velocity.x", &imu_angular_velocity_[0]));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
      n, "angular_velocity.y", &imu_angular_velocity_[1]));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
      n, "angular_velocity.z", &imu_angular_velocity_[2]));

    state_interfaces.emplace_back(hardware_interface::StateInterface(
      n, "linear_acceleration.x", &imu_linear_acceleration_[0]));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
      n, "linear_acceleration.y", &imu_linear_acceleration_[1]));
    state_interfaces.emplace_back(hardware_interface::StateInterface(
      n, "linear_acceleration.z", &imu_linear_acceleration_[2]));
  }
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & period)
{
  ...
  // NEW: pull a fresh IMU sample every read() cycle. On a failed/garbled
  // parse we simply keep the previous values rather than zeroing them out,
  // so one dropped serial frame doesn't inject a spike into the EKF.
  if (!cfg_.imu_sensor_name.empty())
  {
    double gx, gy, gz, ax, ay, az;
    if (comms_.read_imu_values(gx, gy, gz, ax, ay, az))
    {
      imu_angular_velocity_[0] = gx;
      imu_angular_velocity_[1] = gy;
      imu_angular_velocity_[2] = gz;
      imu_linear_acceleration_[0] = ax;
      imu_linear_acceleration_[1] = ay;
      imu_linear_acceleration_[2] = az;
    }
  }
